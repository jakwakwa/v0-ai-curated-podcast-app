...
const bucket = storage.bucket(
  process.env.GOOGLE_CLOUD_STORAGE_BUCKET_NAME as string
);
...
export const generatePodcast = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const summary = await step.run('summarize-content', async () => {
      ...
      console.log(
        'GEMINI_API_KEY at summarize-content step:',
        geminiApiKey ? 'Key is present' : 'Key is undefined/empty'
      );
    ...
    const publicUrl = await step.run(
      ...
      async () => {
        if (aiConfig.simulateAudioSynthesis) {
          console.log(
            'Simulating audio synthesis and upload to bypass ElevenLabs quota...'
          );
          // Simulate an audio buffer and a public URL to continue the workflow
          const simulatedAudioBuffer = Buffer.from(
            'Simulated audio content for testing purposes.'
          );
