...
export default async function AdminPage() {
	...
	const [isDeletingPodcast, setIsDeletingPodcast] = useState<string | null>(
		null,
	);
	...
	const fetchAvailablePodcasts = async () => {
		try {
	...
	const togglePodcastActive = async (podcast: CuratedPodcast) => {
		try {
			...
			toast.success(
				`Podcast ${updatedPodcast.isActive ? "activated" : "deactivated"} successfully!`,
			);
		} catch (error) {
			console.error("Error updating podcast:", error);
			toast.error(
		...
		<div className="container mx-auto p-6 max-w-6xl">
			<div className="mb-8">
				...
				<p className="text-muted-foreground">
					Manage curated content and generate weekly podcast episodes
				</p>
			</div>

			<Tabs defaultValue="episode-generation" className="space-y-6">
				<TabsList className="grid w-full grid-cols-3">
					<TabsTrigger value="episode-generation">
						Episode Generation
					</TabsTrigger>
					<TabsTrigger value="bundle-management">Bundle Management</TabsTrigger>
					<TabsTrigger value="podcast-management">
						Podcast Management
					</TabsTrigger>
				</TabsList>
					...
					<div className="grid gap-6">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Choose which curated bundle to generate an episode for
								</CardDescription>
							</CardHeader>
							<CardContent>
								<Select
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Provide basic information for the episode
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div>
									...
									<Input
										...
										placeholder="e.g., Tech Weekly - January 15, 2024"
									/>
								</div>
								<div>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Add YouTube videos or other sources for each show in the
									bundle
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										...
										<Input
											...
											placeholder="e.g., Lex Fridman - AI Discussion"
										/>
									</div>
									<div>
				...
				<TabsContent value="bundle-management" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Create new bundles or remove existing ones
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<Button
							...
							{showCreateBundle && (
								<div className="space-y-4 p-4 border rounded-lg bg-muted/50">
									...
									<div>
										...
										<div className="mt-2 max-h-60 overflow-y-auto border rounded-lg p-3 space-y-2">
											{availablePodcasts
												...
												.map((podcast) => (
													...
													>
														<Checkbox
															id={`bundle-podcast-${podcast.id}`}
															checked={selectedPodcastIds.includes(podcast.id)}
															onCheckedChange={() =>
																togglePodcastSelection(podcast.id)
															}
														/>
														<div className="flex-1">
															<label
																htmlFor={`bundle-podcast-${podcast.id}`}
										...
										<p className="text-xs text-muted-foreground mt-1">
											Selected: {selectedPodcastIds.length} podcasts
										</p>
									</div>
				...
				<TabsContent value="podcast-management" className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Create, edit, and manage curated podcasts
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<Button
							...
							{showCreatePodcast && (
								<div className="space-y-4 p-4 border rounded-lg bg-muted/50">
									...
									<div>
										<Label htmlFor="podcastImageUrl">
											Image URL (Optional)
										</Label>
										<Input
											id="podcastImageUrl"
											value={newPodcastImageUrl}
											onChange={(e) => setNewPodcastImageUrl(e.target.value)}
									...
									<Button
										onClick={editingPodcast ? updatePodcast : createPodcast}
										disabled={isCreatingPodcast || isUpdatingPodcast}
										className="w-full"
									>
						...
						<Card key={category}>
							...
							<CardContent>
								<div className="space-y-3">
									{podcastsByCategory[category].map((podcast) => (
										...
										>
											...
											<div className="flex items-center gap-2 ml-4">
												<Button
													onClick={() => togglePodcastActive(podcast)}
													variant="ghost"
													size="sm"
													title={podcast.isActive ? "Deactivate" : "Activate"}
												>
													{podcast.isActive ? (
									...
									{podcastsByCategory[category].length === 0 && (
										<p className="text-center text-muted-foreground py-4">
											No {category.toLowerCase()} podcasts yet.
										</p>
									)}
								</div>
							</CardContent>
