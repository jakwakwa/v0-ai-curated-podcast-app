...
import { HomePageBackground } from "@/components/gradients/page-backgrounds";
...
function ProtectedLayoutInner({ children }: { children: React.ReactNode }) {
	...
	return (
		<>
			...
			<SidebarInset>
				<HomePageBackground />
				<header className={`fixed bg-sidebar flex h-16 backdrop-blur-[10px] shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12 mt-0 w-screen justify-between px-2 md:px-4 py-0 overflow-y-scroll z-50 ${state === "expanded" ? "" : ""}`} >
					<div className={`flex items-center h-16 justify-between gap-2 px-2  ${state === "expanded" ? "md:px-4" : "md:px-0"}`}>
						...
						<Separator orientation="vertical" className="data-[orientation=vertical]:h-4 border-[#D5C7B739] border-[1px] ml-4 ">{""}</Separator>
						<DynamicBreadcrumb />
				...
				<div className={`flex flex-col flex-grow transition-all duration-300 ease-in-out px-0 md:px-0 mt-8 md:mt-0 mb-2 m-0 p-0 h-screen ${state === "expanded" ? "ml-0 w-full md:ml-6 md:max-w-[80vw]" : "ml-0 md:ml-12 w-full md:max-w-[95vw]"}`}>
