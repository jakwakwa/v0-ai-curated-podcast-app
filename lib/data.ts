...
import type { Episode, UserCurationProfile, UserCurationProfileStatus, Source, CuratedPodcast, TransformedCuratedBundle } from '@/lib/types'
import type { Source as PrismaSource, Episode as PrismaEpisode, UserCurationProfile as PrismaUserCurationProfile, CuratedBundle as PrismaCuratedBundle, CuratedPodcast as PrismaCuratedPodcast } from "@prisma/client"
...
const DUMMY_TRANSFORMED_BUNDLE: TransformedCuratedBundle = {
  ...
  createdAt: new Date(),
  podcasts: [
    DUMMY_CURATED_PODCASTS[0],
    DUMMY_CURATED_PODCASTS[1],
  ],
};
...
const DUMMY_USER_CURATION_PROFILES: UserCurationProfile[] = [
  {
    ...
    user: {} as any, // Placeholder, not used in this context
...
const DUMMY_EPISODES: Episode[] = [
  ...
  },
  {
    ...
    source: { // Dummy source for this episode as it's from a bundle
      id: "src3",
      collectionId: DUMMY_USER_CURATION_PROFILES[1].id,
...
export async function getCuratedCollections(): Promise<UserCurationProfile[]> {
	...
	// })
  console.log("Using DUMMY_USER_CURATION_PROFILES (Temporary)");
  return DUMMY_USER_CURATION_PROFILES;
}
