	...
	for (let attempt = 0; attempt <= retries; attempt++) {
		try {
			...
			for await (const chunk of response) {
				const inlineData = chunk.candidates?.[0]?.content?.parts?.[0]
...
export const generateUserEpisodeMulti = inngest.createFunction(
	{
		...
		onFailure: async ({ event }) => {
			const { userEpisodeId } = (
				event as unknown as { data: { userEpisodeId: string } }
			).data;
...
Summary: ${summary}`,
		...
		const gcsAudioUrl = await step.run(
			'synthesize-multi-voice-and-upload',
			async () => {
				const chunks: Buffer[] = [];
		...
		await step.run('extract-duration', async () => {
			...
			if (!result.success) {
				console.warn(
					`[DURATION_EXTRACTION] Failed to extract duration: ${result.error}`
				);
			}
			return result;
		});
