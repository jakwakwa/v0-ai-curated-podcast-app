...
const PaystackControls = dynamic(
  () => import("./subscription-test-controls.paystack.disabled").then((mod) => mod.SubscriptionTestControls),
  { ssr: false }
)

const PaddleControls = dynamic(
  () => import("./subscription-test-controls-paddle").then((mod) => mod.SubscriptionTestControlsPaddle),
  { ssr: false }
)
