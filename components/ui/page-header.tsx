...
const PageHeader = React.forwardRef<HTMLDivElement, PageHeaderProps>(({ className, spacing, title, description, level = 1, ...props }, ref) => {
	...
	return (
		<div className="bg-[#3a517c4f] page-header-bg rounded-2xl px-2 flex flex-col justify-between">
			<div className={cn(headerVariants({ spacing, className }))} ref={ref} {...props}>
				<h2 className="flex text-xl font-bold px-2 md:px-0 pt-0 pb-0 md:py-0 text-shadow-sm text-primary-forefround leading-[1.5] max-w-screen lg:max-w-4xl">{title}</h2>
				{description && (
