...
const PageHeader = React.forwardRef<HTMLDivElement, PageHeaderProps>(({ className, spacing, title, description, level = 1, ...props }, ref) => {
	...
	return (
		<div className={cn(headerVariants({ spacing, className }))} ref={ref} {...props}>
			<h2 className="flex text-custom-h2 font-medium px-2 md:px-0 pt-0 pb-0 md:py-0	 text-primary leading-[1.5] max-w-screen md:max-w-4xl">
				{title}
			</h2>
			{description && (
				<Typography as="p" variant="body" className="text-base px-2  md:px-0  md:py-1.5 text-primary/60 leading-[1.5] max-w-screen md:max-w-4xl">
					{description}
				</Typography>
			)}
		</div>
