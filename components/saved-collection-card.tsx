...
export function SavedCollectionCard({
  ...
  const formattedTime = currentCollection.createdAt.toLocaleTimeString(
    'en-ZA',
    {
      hour: '2-digit',
      minute: '2-digit',
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      pollingInterval = setInterval(async () => {
        const updatedCollection = await getCollectionStatus(
      ...
      setTimeout(() => {
        if (isLoading && pollingInterval) {
          ...
          toast({
            ...
            description:
              'Podcast generation is taking longer than expected. Please check back later.',
            variant: 'destructive',
          });
        }
      ...
      toast({
        title: 'Error',
        description: 'Failed to initiate podcast generation.',
        variant: 'destructive',
      });
      setIsLoading(false);
      if (pollingInterval) clearInterval(pollingInterval);
