...
describe("curated bundles API", () => {
	...
	it("sets canInteract=false for higher gate when non-admin, true for admin; returns podcasts list", async () => {
		...
		let res = await GET(new Request("http://test.local"))
		let data = await res.json()
		let item = data.find((x: any) => x.bundle_id === b.bundle_id)
		...
		res = await GET(new Request("http://test.local"))
		data = await res.json()
		item = data.find((x: any) => x.bundle_id === b.bundle_id)
